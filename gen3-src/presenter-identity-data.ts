...
Workflow ที่บังคับ
1. ถ้าเป็นภาพใหม่ไม่มี reference ให้ใช้ Kie API โมเดล gpt-image-2-text-to-image
2. ถ้ามีภาพตัวตน สไตล์ หรือเฟรมเดิมเป็น reference ให้ใช้ gpt-image-2-image-to-image และรักษา reference เป็นแหล่งความจริง
3. บันทึก task ID, model ID, prompt, result URL, ไฟล์, credits และ hash ใน manifest หากระบบที่ทำงานรองรับ
4. ตรวจภาพเต็มความละเอียดทั้ง 6 ช่อง หากหน้า อายุ ผม ชุด สัดส่วน มือ หรือความต่อเนื่องคลาดเคลื่อน ให้สร้างใหม่ ห้ามแก้ข้อความไทยหรือปะใบหน้าด้วยโปรแกรม
...
ห้ามสร้างรูปจริงถ้าไม่มีเครื่องมือ Kie ที่ได้รับอนุญาต ให้ส่งเพียง Prompt พร้อมใช้และห้ามอ้างว่าสร้างภาพแล้ว`;
}
...
- ภาพฉากที่อ้างอิงตัวละครต้องใช้ Kie API gpt-image-2-image-to-image เมื่อระบบมีเครื่องมือและสิทธิ์ ห้ามใช้ text-to-image แล้วหวังให้หน้าคงเดิม
